...
const users: { id: number; email: string; password: string; role: 'user' | 'admin'; loginAttempts: number; blockedUntil: Date | null }[] = [];
let nextId = 1;
  ...
  try {
    ...
    if (!isValid) {
      ...
      if (user.loginAttempts >= 5) {
        user.blockedUntil = new Date(Date.now() + 15 * 60 * 1000); // 15 minutos
